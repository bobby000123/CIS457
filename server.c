/*
	...
	Description : This program comes paired with a client
	> program which facilitates the communication between one
	> or multiple users of a peer-to-peer based chatroom. This
	> program is the server-side which faciltates the peer-to-peer
	> connection between two clients (users).

	Usage: 
	> compile and run 1 instance server.c with -lpthread
	> compile and run multiple instances of client.c with -lpthread
	> set one client to wait for a connnection
	> set at least one other client to connect
	> connect to the desired client using the
	list provided
